...
### create a new enviroment for one condition
env_c10p9 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped20k/c10p9/modelSmr.Rdata", envir = env_c10p9)


meanDiffLL_mtam_c10p9 <- env_c10p9$smr2$Minus2LogLikelihood - env_c10p9$smr1$Minus2LogLikelihood
PN <- ncol(env_c10p9[["smr1"]][["dataSummary"]][["fam1"]])*length(env_c10p9[["smr1"]][["dataSummary"]])
lamdaUnit_c10p9 <- meanDiffLL_mtam_c10p9/PN
SSize_c10p9 <- 1: demo_n
LamdaVec_c10p9 <- lamdaUnit_c10p9*SSize_c10p9
powVec_c10p9 <- as.numeric(lapply(LamdaVec_c10p9,powerCal, df = 1))

df_c10p9 <- data.frame(Nped = SSize_c10p9, 
                      power = powVec_c10p9, 
                      Combination = rep("power4", demo_n))

### create a new enviroment for one condition
env_c10p10 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped20k/c10p10/modelSmr.Rdata", envir = env_c10p10)


meanDiffLL_mtam_c10p10 <- env_c10p10$smr2$Minus2LogLikelihood - env_c10p10$smr1$Minus2LogLikelihood
PN <- ncol(env_c10p10[["smr1"]][["dataSummary"]][["fam1"]])*length(env_c10p10[["smr1"]][["dataSummary"]])
lamdaUnit_c10p10 <- meanDiffLL_mtam_c10p10/PN
SSize_c10p10 <- 1: demo_n
LamdaVec_c10p10 <- lamdaUnit_c10p10*SSize_c10p10
powVec_c10p10 <- as.numeric(lapply(LamdaVec_c10p10,powerCal, df = 1))

df_c10p10 <- data.frame(Nped = SSize_c10p10, 
                       power = powVec_c10p10, 
                       Combination = rep("power5", demo_n))

### create a data frame for graphs
df_c10 <- rbind(df_c10p2, df_c10p6, df_c10p8,df_c10p9,df_c10p10)
...
ggsave( "~/R-Project/mtDNA_mt2/Result_rdped20k/Analysis_rdped20k/graph_c10p268910.png",g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
