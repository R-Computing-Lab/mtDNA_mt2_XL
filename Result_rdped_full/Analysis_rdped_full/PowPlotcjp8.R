...
### create a new enviroment for one condition
env_c15p8 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped_full/c15p8/modelSmr.Rdata", envir = env_c15p8)


meanDiffLL_mtam_c15p8 <- env_c15p8$smr2$Minus2LogLikelihood - env_c15p8$smr1$Minus2LogLikelihood
PN <- ncol(env_c15p8[["smr1"]][["dataSummary"]][["fam1"]])*length(env_c15p8[["smr1"]][["dataSummary"]])
lamdaUnit_c15p8 <- meanDiffLL_mtam_c15p8/PN
SSize_c15p8 <- 1: demo_n
LamdaVec_c15p8 <- lamdaUnit_c15p8*SSize_c15p8
powVec_c15p8 <- as.numeric(lapply(LamdaVec_c15p8,powerCal, df = 1))

df_c15p8 <- data.frame(Nped = SSize_c15p8, 
                       power = powVec_c15p8, 
...
ggsave( "~/R-Project/mtDNA_mt2/Result_rdped_full/Analysis_rdped_full/graph_cjp8.png",g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
