...
### create a new enviroment for one condition
env_c12p8 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped_full/c12p8/modelSmr.Rdata", envir = env_c12p8)


meanDiffLL_mtam_c12p8 <- env_c12p8$smr2$Minus2LogLikelihood - env_c12p8$smr1$Minus2LogLikelihood
PN <- ncol(env_c12p8[["smr1"]][["dataSummary"]][["fam1"]])*length(env_c12p8[["smr1"]][["dataSummary"]])
lamdaUnit_c12p8 <- meanDiffLL_mtam_c12p8/PN
SSize_c12p8 <- 1: demo_n
LamdaVec_c12p8 <- lamdaUnit_c12p8*SSize_c12p8
powVec_c12p8 <- as.numeric(lapply(LamdaVec_c12p8,powerCal, df = 1))

df_c12p8 <- data.frame(Nped = SSize_c12p8, 
                      power = powVec_c12p8, 
...
ggsave( "~/R-Project/mtDNA_mt2/Result_rdped_full/Analysis_rdped_full/graph_ccnp8.png",g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
