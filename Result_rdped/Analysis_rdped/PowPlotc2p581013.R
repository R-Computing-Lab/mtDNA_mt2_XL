...
env_c2p5 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped/c2p5/modelSmr.Rdata", envir = env_c2p5)
...
### create a new enviroment for one condition
env_c2p10 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped/c2p10/modelSmr.Rdata", envir = env_c2p10)
...
### create a new enviroment for one condition
env_c2p13 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped/c2p13/modelSmr.Rdata", envir = env_c2p13)


meanDiffLL_mtam_c2p13 <- env_c2p13$smr2$Minus2LogLikelihood - env_c2p13$smr1$Minus2LogLikelihood
PN <- ncol(env_c2p13[["smr1"]][["dataSummary"]][["fam1"]])*length(env_c2p13[["smr1"]][["dataSummary"]])
lamdaUnit_c2p13 <- meanDiffLL_mtam_c2p13/PN
SSize_c2p13 <- 1: demo_n
LamdaVec_c2p13 <- lamdaUnit_c2p13*SSize_c2p13
powVec_c2p13 <- as.numeric(lapply(LamdaVec_c2p13,powerCal, df = 1))

df_c2p13 <- data.frame(Nped = SSize_c2p13, 
                      power = powVec_c2p13, 
                      Combination = rep("power4", demo_n))

### create a data frame for graphs
df_c2 <- rbind(df_c2p5, df_c2p8, df_c2p10,df_c2p13)
...
ggsave( "~/R-Project/mtDNA_mt2/Result_rdped/Analysis_rdped/graph_c2p581013.png",g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
