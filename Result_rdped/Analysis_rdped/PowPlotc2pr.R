...
### create a new enviroment for one condition
env_c2p12 <- new.env()
load("~/R-Project/mtDNA_mt2/Result_rdped/c2p12/modelSmr.Rdata", envir = env_c2p12)


meanDiffLL_mtam_c2p12 <- env_c2p12$smr2$Minus2LogLikelihood - env_c2p12$smr1$Minus2LogLikelihood
PN <- ncol(env_c2p12[["smr1"]][["dataSummary"]][["fam1"]])*length(env_c2p12[["smr1"]][["dataSummary"]])
lamdaUnit_c2p12 <- meanDiffLL_mtam_c2p12/PN
SSize_c2p12 <- 1: demo_n
LamdaVec_c2p12 <- lamdaUnit_c2p12*SSize_c2p12
powVec_c2p12 <- as.numeric(lapply(LamdaVec_c2p12,powerCal, df = 1))

df_c2p12 <- data.frame(Nped = SSize_c2p12, 
                       power = powVec_c2p12, 
...
ggsave( "~/R-Project/mtDNA_mt2/Result_rdped/Analysis_rdped/graph_c2pr.png",g1, width = 6, height = 4.5,  type = "cairo-png", dpi = 900)
